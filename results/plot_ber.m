...
semilogy(snr,ber1,'-o',snr,ber2,'-v',snr,ber3,'-s',...
         snr,ber4,'-d', snr, ber5, '-^', 'LineWidth', 1.5);
title('BER for \pi/2 BPSK MCSs in AWGN Channel'); grid on;
...
semilogy(snr,ber6,'-o',snr,ber7,'-v',snr,ber8,'-s',...
         snr,ber9,'-d', 'LineWidth', 1.5);
title('BER for \pi/2 QPSK MCSs in AWGN Channel'); grid on;
xlabel('SNR (dB)'); ylabel('Bit Error Rate');
